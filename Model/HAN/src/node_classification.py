...
def nc_evaluate(dataset, supervised, label_file_path, label_test_path, emb_dict):    
    ...
    if supervised=='True':
        ...
        elif dataset=='DBLP' or dataset=='Freebase' or dataset=='PubMed' or dataset=='ACM':
            #我的
...
def semisupervised_single_class_single_label(label_file_path, label_test_path, emb_dict):
    ...
    test_labels111 = torch.from_numpy(test_labels)

    _, indices = torch.max(test_embeddings, dim=1)
    prediction = indices.long().cpu().numpy()
    # test_labels = test_labels.cpu().numpy()

    label_loss = F.nll_loss(F.log_softmax(test_embeddings, dim=1), test_labels111.long())
    ...
    # # print(preds)
    # predsp = clf._predict_proba_lr(test_embeddings)
    # # print(predsp)

    # preds111 = torch.from_numpy(predsp)
    # test_labels111 = torch.from_numpy(test_labels)

    # loss = F.nll_loss(F.log_softmax(preds111, dim=1), test_labels111.long())
    # # print('loss',loss.item())
    # accuracy = (preds == test_labels).sum() / len(preds)
    ...
    return macro_f1, micro_f1, accuracy, label_loss.item()
